...
func (eq *eventsQueue) Put(newEvent *types.Event) error {
...
// but was contained
func (eq *eventsQueue) Get() (*types.Event, error) {
	eq.mutex.Lock()
	defer eq.mutex.Unlock()
	if eq.head == nil {
		if eq.tail != nil {
...
func (eq *eventsQueue) PeekHead() *types.Event {
	eq.mutex.Lock()
	defer eq.mutex.Unlock()
	if eq.head == nil {
		return nil
...
func (eq *eventsQueue) PeekTail() *types.Event {
	eq.mutex.Lock()
	defer eq.mutex.Unlock()
	if eq.tail == nil {
		return nil
