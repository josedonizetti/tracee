...
func (b *Broadcast) Print(event v1beta1.Event) {
...
func startPrinter(wg *sync.WaitGroup, done chan struct{}, c chan v1beta1.Event, p EventPrinter) {
	for {
		select {
		case <-done:
			wg.Done()
