...
func (c *BuffersConfig) flags() []string {
	...
	if c.Kernel.Blobs != 0 {
		flags = append(flags, fmt.Sprintf("kernel.blobs=%d", c.Kernel.Blobs))
	}
...
func PrepareBuffers(flags []string) (BuffersConfig, error) {
	...
	for _, flag := range flags {
		...
		case kernelBlobsFlag:
			buffers.Kernel.Blobs = size
