...
func (s *Stream) publish(ctx context.Context, event *v1beta1.Event) {
...
func (s *Stream) shouldIgnorePolicy(event *v1beta1.Event) bool {
	// return s.policyMask&event.MatchedPoliciesUser == 0
	// return s.policyMask&event.GetPolicies().GetMatched() == 0
	// TODO: needs review (josedonizetti)
	return false
}
...
func (sm *StreamsManager) Subscribe(policyMask uint64, chanSize int) *Stream {
	...
	stream := &Stream{
		...
		events:     make(chan *v1beta1.Event, chanSize),
	}
...
func (sm *StreamsManager) Publish(ctx context.Context, event *v1beta1.Event) {
	sm.mutex.Lock()
	defer sm.mutex.Unlock()
